...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round((df_min_workers["salary"] == ">50K").mean() * 100, 1)

    # What country has the highest percentage of people that earn >50K?
    country_count = df["native-country"].value_counts()
    rich_country_count = df[df["salary"] == ">50K"]["native-country"].value_counts()
    rich_percent = (rich_country_count / country_count) * 100

    highest_earning_country = rich_percent.sort_values(ascending=False).index[0]
    highest_earning_country_percentage = round(rich_percent.sort_values(ascending=False).iloc[0], 1)

    # Identify the most popular occupation for those who earn >50K in India.
    df_india_rich = df[(df["native-country"] == "India") & (df["salary"] == ">50K")]
    top_IN_occupation = df_india_rich["occupation"].mode()[0]
